   ...
   public static void main(String[] args)
   {
      ...
      buildContent(
         frame, panel()
            ...
            .add(line()
                    .add("Label respecting baseline")
                    .add(textField)
            )
            .add(line()
                    .add("&Label respecting baseline", spec().withBaseline().withAnchorY(AY.CENTER))
                    .add(vpanel()
                            .withContentAnchor(A.BOTH)
                            .withLineLength(4)
                            .addAll(Arrays.asList(checkBoxes), spec().withIPadY(10).withInsetTop(10))
                    )
            )
      );
